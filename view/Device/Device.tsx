  ...
  const tabs: TabsContProps[] = [
    {
      ...
      content: device.specifications?.map(specification => <div key={specification.title} className={styles.specificationTitle}>
        <div className={styles.specificationTitle} style={{fontWeight: 700}}>{specification.title}</div>
        {specification.items.map(item => <div key={item.label} className={styles.specificationLabekAndValueCont}>
    ...
    },
    {
      ...
      content: <a href={device.passportUrl} download={device.passportUrl?.slice(10)} className={styles.url}><Image src={DownloadIcon} alt="" className={styles.downloadIcon}/><div className={styles.downloadText}>Паспорт продукта</div></a>,
    },
  ];
    ...
    <>
      <div className={styles.ImgAndDescriptionContainer}>
        ...
        <div className={styles.descriptioinContainer}>
          ...
          <ButtonLink href={"tel:" + siteConfig.telephone}><div>{siteConfig.telephone}</div></ButtonLink>
          <div className={styles.description}>{device.info}</div>
        </div>
      </div>
      <div className={styles.DesktopTabs}>
      <Tabs aria-label="Dynamic tabs" items={tabs} radius="none" variant="light" className={styles.tabsCont}>
        {(item) => (
